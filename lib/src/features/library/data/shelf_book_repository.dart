...
enum ShelfBookSortBy {
  ...
  /// Manual order: the user arranges items by long-press dragging.
  /// The order is stored per shelf container (see BookshelfNotifier).
  custom,
}
...
class ShelfBookRepository {
  ...
  Future<List<ShelfBook>> getBooksSorted({
    ...
    switch (sortBy) {
      ...
      case ShelfBookSortBy.custom:
        // custom uses the DB's natural order; the notifier applies the
        // user's manual order on top (see buildShelfItems).
