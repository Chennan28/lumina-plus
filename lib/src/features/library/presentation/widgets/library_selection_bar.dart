...
class LibrarySelectionBar extends StatelessWidget {
  ...
  /// Merges the selected books into a series (enabled when at least two books
  /// are selected, or when books + exactly one series are selected).
  final VoidCallback onMerge;

  /// Merge is enabled when the user picked at least two books, or picked
  /// books together with exactly one series (the books join that series).
  bool get canMerge =>
      state.selectedBookIds.length >= 2 ||
      (state.selectedBookIds.isNotEmpty && state.selectedSeriesIds.length == 1);
          ...
          child: Row(
            ...
            children: [
              ...
              _ActionButton(
                ...
                label: AppLocalizations.of(context)!.mergeIntoSeries,
